...
const createReview = async (req, res) => {
    ...
    try {
        ...
        let findBlog= await blogModel.findById({_id: userId})
        if(!findBlog) return res.status(400).send({ status: false, message: `blog not found, create new one` });

        let findExistUserId= await reviewModel.findOne({userId})
        if(findExistUserId) return res.status(400).send({ status: false, message: `Review for this blog Already Exist, Please update` });
